      ...
      <div className={`lonyo-menu-wrapper ${menuOpen ? 'lonyo-body-visible' : ''}`}>
        <div className="lonyo-menu-area text-center">
          ...
          <div className="lonyo-mobile-menu">
            <ul>
              {menu_data.map((item, i) => (
                <li
                  key={i}
                  className={`${item.has_dropdown ? 'menu-item-has-children lonyo-item-has-children' : ''} ${navTitle === item.title ? 'lonyo-active' : ''}`}>
                  ...
                  {item.has_dropdown && (
                    <ul className="sub-menu" style={{ display: navTitle === item.title ? 'block' : 'none' }}>
                      {item.sub_menus?.map((sub_item, index) => (
                        <li
                          key={index}
                          className={`menu-item-has-children lonyo-item-has-children ${navTitle2 === sub_item.title ? 'lonyo-active' : ''}`}>
                          <Link to={sub_item.link} onClick={() => openMobileMenu2(sub_item.title)}>
                            {sub_item.title}
                          ...
                          {sub_item.inner_submenu && (
                            <ul
                              ...
                              {sub_item.sub_menu?.map((inner_sub_item, inner_index) => (
                                <li key={inner_index}>
                                  <Link to={inner_sub_item.link}>{inner_sub_item.title}</Link>
                                </li>
                              ))}
                            </ul>
                          )}
          ...
          <div className="lonyo-mobile-menu-btn">
            <Link className="lonyo-default-btn sm-size" to="/contact-us" data-text="Get in Touch">
