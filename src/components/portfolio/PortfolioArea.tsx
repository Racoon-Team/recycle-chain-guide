    ...
    <>
      <div className="lonyo-section-padding9">
        <div className="container">
          <div className="lonyo-portfolio-menu lonyo-section-title" data-aos="fade-up" data-aos-duration="500">
            <ul id="watch-filter-gallery" className="option-set clear-both" data-option-key="filter">
              {categories.map((cate, i) => (
                <li
                  key={i}
                  onClick={() => filterItems(cate)}
                  className={`wow fadeInUpX ${cate === activeCategory ? 'active' : ''}`}>
                  {cate}
                </li>
              ))}
            </ul>
          ...
          <div className="lonyo-portfolio-column" id="lonyo-portfolio-grid">
            <ResponsiveMasonry columnsCountBreakPoints={{ 0: 1, 750: 2, 992: 2 }}>
              <Masonry>
                {items.map((item, i) => (
                  <div key={i} className="lonyo-p-thumb" data-aos="fade-up" data-aos-duration="700">
                      ...
                      <div className="lonyo-p-content">
                        ...
                        <ul>
                          {item.tags.map((tag, index) => (
                            <li key={index}>
                              <Link to="/single-portfolio">{tag}</Link>
                            </li>
                          ))}
                        </ul>
                      </div>
